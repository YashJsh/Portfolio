...
export default function ContactSection() {
    return (
        <div className="flex flex-col items-center justify-center pt-4 pb-8 text-center relative z-10 w-full mt-2">
            ...
            <h2 className="text-3xl sm:text-5xl md:text-6xl font-extrabold tracking-[-0.06em] mb-4">
                Get in Touch
            </h2>
            
            <div className="text-base sm:text-lg md:text-xl text-muted-foreground mb-6 flex flex-col items-center justify-center gap-y-1 font-medium px-2">
                <div className="flex items-center flex-wrap justify-center gap-y-1">
                    <span>Want to chat? Just shoot me a dm via</span>
                    <a 
                        href={`mailto:${DATA.contact.email}`} 
                        className="inline-flex items-center gap-2 px-3 py-1.5 mx-1 border border-dashed border-border/80 rounded-lg bg-background hover:bg-muted/50 hover:border-foreground/50 transition-colors text-foreground text-sm sm:text-base shadow-sm"
                    >
            ...
            <div className="flex flex-col items-center gap-2 mt-2 w-full">
                ...
                <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-4 text-muted-foreground/50">
                    <button 
                        onClick={() => {
                            navigator.clipboard.writeText("YashYpPCggkGGLe7mmFcfLLWLzKGFGbFED9S1UBdAnZ");
